...
# ---------------------------------------------------------------------
r"""
Quantization script for Qwen3-4B.
...
def export_onnx(
    ...
    fp_model = Qwen3_4B_PreSplit.from_pretrained(
    ...
    Qwen3_4B_QuantizablePreSplit.create_onnx_models(
    ...
    Qwen3_4B_QuantizablePreSplit.save_tokenizer_and_config(
...
def quantize(
    ...
    print("Starting quantization for Qwen3-4B")
    print(f"  Context length: {context_length}")
    print(f"  Sequence length: {seq_len}")
    ...
    if onnx_dir:
        ...
        model_quant = Qwen3_4B_QuantizablePreSplit.from_pretrained(
            checkpoint=onnx_dir,
            fp_model=fp_model,
            precision=precision,
            host_device=host_device,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Quantize Qwen3-4B model",
    )
